...
* 成功启动xv6操作系统，掌握使用Git进行版本控制、编译操作系统代码，并通过QEMU模拟环境运行xv6的基本操作，初步理解操作系统启动过程和文件系统交互。

#### 实验步骤

##### 安装`vmware`虚拟机

* 这里安装的是最新版的`Vmware Workstation`（破解版），也可以选择其它的虚拟机；

##### 安装`Ubuntu`操作系统

* 选择`Ubuntu`是它对`Linux`初学者比较友好，操作起来跟`Windows`很像；
...
* 安装课程所需的工具，包括 Git、Build-essential（用于编译工具）、多架构支持的 GDB、QEMU、RISC-V 版本的 GCC 和 Binutils。执行以下命令：
...
* 这个命令将安装以下内容：
  - **git**：用于版本控制和管理代码。
  - **build-essential**：包含了编译 C/C++ 代码所需的基本工具。
  - **gdb-multiarch**：支持多架构的 GDB 调试器，包括 RISC-V 架构。
  - **qemu-system-misc**：QEMU 系统仿真器，支持多种架构的仿真。
  - **gcc-riscv64-linux-gnu**：RISC-V 架构的 GCC 编译器。
  - **binutils-riscv64-linux-gnu**：RISC-V 架构的二进制工具集（如汇编器、链接器等）。
* 验证安装：

```bash
git --version
gcc-riscv64-linux-gnu --version
...
* 在虚拟机上完成实验，所以虚拟机应该能连接到`github`网站，但是一开始虚拟机并不能正常使用主机的`VPN`；

###### 解决方案

* `Google`搜索相关经验贴，调整虚拟机的网络设置和`Ubuntu`内的有线设置，成功解决；



#### 实验心得

* 通过本次实验，我深入了解了xv6操作系统的启动流程，熟悉了Git版本控制的基本操作，并成功在QEMU环境中运行了xv6。这次实验不仅巩固了我对操作系统基础知识的理解，还提升了我的实际操作能力。
...
* 一开始使用`strol`作为把字符串转换为整数的工具，但是在`xv6`的环境下这个函数似乎并没有被实现，所以报错了；
...
* 通过此次实验，我成功实现了一个用户级的 `sleep` 程序，类似于UNIX系统中的 `sleep` 命令。这个程序可以根据用户指定的时间间隔（以时钟周期为单位）暂停执行。实验过程中，我首先仔细阅读了xv6的第一章内容，了解了操作系统中的时间管理机制，并参考了 `user/` 目录下的其他程序如 `echo.c` 和 `grep.c`，学习了如何在xv6中处理命令行参数。

  在实际编写代码时，我使用了 `atoi` 函数将命令行输入的字符串参数转换为整数，并通过调用 `sleep` 系统调用来实现程序的暂停功能。通过将代码集成到Makefile的UPROGS中，我得以在xv6的shell中运行并验证我的程序。在程序结束时，我确保通过调用 `exit(0)` 正常退出。

  此次实验让我深入理解了xv6中的时间管理和系统调用机制，同时也熟悉了在嵌入式环境中处理命令行参数和错误处理的基本方法。实验的成功让我对操作系统底层实现有了更清晰的认识，也为后续更复杂的实验奠定了坚实的基础。
...
* 在这个实验中，我实现了一个基于并发管道的素数筛选程序，用于在xv6操作系统上通过管道和多进程机制筛选2到35之间的素数。这个设计灵感源于Unix管道的发明者Doug McIlroy。

  实验的核心在于使用 `pipe` 和 `fork` 来建立一个进程管道，其中每个进程负责筛选一个素数。第一个进程将数字2到35依次输入管道。对于每个素数，我设计了一个进程，该进程从其左侧的邻居进程读取数据，并通过另一个管道将结果传递给右侧的邻居进程。由于xv6对文件描述符和进程数量有限制，第一个进程只处理到35即可。

  在编写程序的过程中，我注意到需要及时关闭不再使用的文件描述符，以避免资源耗尽的问题。此外，我确保在主进程退出之前，所有子进程及其衍生进程都已正确结束，以确保输出的完整性和程序的正确性。

  通过这次实验，我不仅加深了对并发编程的理解，还学会了在受限资源环境下如何有效管理进程和文件描述符。这次实验让我深刻体验了xv6操作系统中的进程管理和管道通信机制，并为后续更复杂的并发编程任务奠定了坚实的基础。
...
* 没有正确并及时的关闭不再需要的文件描述符，导致进程不能正常推进；

##### 解决方法

* 参考别人的代码，正确关闭不需要的文件描述符。
...
![将find添加到Makefile](img/将find添加到Makefile.png)

##### 测试成功

![find-test](img/find-test.png)
...
* 在这个实验中，我编写了一个简化版的UNIX `find` 程序，用于在xv6操作系统中搜索目录树中的特定文件。该程序递归地遍历目录及其子目录，查找与指定名称匹配的文件。

  在实现过程中，我参考了 `user/ls.c` 的代码来了解如何读取目录内容，并通过递归函数实现目录的深度遍历。为了避免进入当前目录 `.` 和父目录 `..`，我在递归时进行了适当的过滤处理。此外，我学习并使用了C语言中的 `strcmp()` 函数来比较字符串，而不是直接使用 `==`，以确保字符串比较的正确性。

  实验中还涉及到文件系统的持久化处理。通过运行 `make clean` 和 `make qemu`，我确保了在每次测试前都使用干净的文件系统。最终，我的程序能够正确地在目录树中找到所有符合条件的文件，并输出其路径。

  通过此次实验，我深入理解了递归算法在文件系统遍历中的应用，并强化了我对C语言字符串处理的掌握。这次实验不仅让我熟悉了xv6文件系统的结构和操作，还提升了我在嵌入式操作系统环境下解决问题的能力。

### 6. xargs()(moderate)

#### 实验目的

* 在这个实验中，你需要为 `xv6` 操作系统编写一个简单版本的 UNIX `xargs` 程序。`xargs` 的作用是读取标准输入的行，并为每一行执行给定的命令，逐行将输入行附加到命令的参数后面。
...
xargs(char** first_arg, int size, char* program_name)
{
    ...
    while (read(0, buf+m, 1) == 1) {
        ...
        if (buf[m] == '\n') {
            ...
            // 设置参数索引
            int argIndex = size;
            if (argIndex == 0) { // 如果没有提供初始参数，则将命令名作为第一个参数
...
1. **读取命令行参数**：`main` 函数读取命令行参数，如果提供了命令名，则使用该命令；否则默认使用 `echo`。
2. **读取标准输入**：`xargs` 函数从标准输入逐字符读取输入行，直到遇到换行符（表示一行结束）。
3. **生成命令和参数**：每次读取到一行输入后，将该行作为参数附加到命令的后面，并使用 `exec` 执行命令。
4. **执行命令**：`xargs_exec` 函数创建一个子进程并执行命令，父进程等待子进程完成后继续下一次输入的读取和命令执行。
...
1. 标准输入的数据长度超过缓冲区大小时导致溢出。
2. 管道传递数据时未能正确关闭文件描述符，导致资源泄漏。
3. 使用 `exec` 执行命令时，由于未正确设置参数，导致程序崩溃。

##### 解决方案

1. 为了解决缓冲区溢出的问题，我增加了输入数据的长度检查，并在缓冲区满时输出错误信息并终止程序。
2. 为防止资源泄漏，我在合适的地方确保关闭了所有不再使用的文件描述符。
3. 针对 `exec` 参数设置的问题，我仔细检查了传递给 `exec` 的参数数组，确保每个参数正确赋值，并以空指针结尾，以避免程序崩溃。

#### 实验心得

* 在本次实验中，我成功地实现了一个简化版的 UNIX `xargs` 程序，用于在 xv6 操作系统中读取标准输入的行，并将每一行作为参数传递给指定的命令执行。通过这个实验，我不仅加深了对进程管理和系统调用的理解，还学会了如何在资源受限的环境中高效地处理输入和输出。

  实验过程中，我遇到了缓冲区溢出、文件描述符管理不当、以及 `exec` 参数传递错误等问题。通过增加对输入数据长度的检查、确保及时关闭不再使用的文件描述符、并仔细验证 `exec` 调用的参数数组，我成功解决了这些问题，保证了程序的稳定性和正确性。

  此次实验让我体会到在编写系统级程序时，细致的错误处理和资源管理是至关重要的。通过这个 `xargs` 程序的实现，我不仅巩固了对 C 语言基础的掌握，还提升了编写健壮代码的能力，这对我未来在操作系统和嵌入式系统领域的学习和工作将大有裨益。
